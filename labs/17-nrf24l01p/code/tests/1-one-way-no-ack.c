...
one_way_noack(nrf_t *server_nic, nrf_t *client_nic, int verbose_p) {
    ...
    for(unsigned i = 0; i < ntrial; i++) {
        if(verbose_p && i  && i % 100 == 0)
            output("sent %d no-ack packets\n", i);
        nrf_send_msg_noack(server_nic, client_addr, mk_test_msg(i, nbytes));
    ...
    output("trial: total successfully sent %d no-ack packets lost %d\n",
        npackets, ntimeout);
...
void notmain(void) {
    ...
    output("about to configure no-ack client\n");
    nrf_t *client_nic = client_config(nbytes, ack_p);
    output("about to configure no-ack server\n");
